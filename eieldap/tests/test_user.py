...
def test_ntlm_passwords_generated_automatically():
    user = User('Gunea', 'Pig', 1, password=valid_password)
...
def test_password_must_have_length_greater_than_6():
    with pytest.raises(TypeError):
        User('Gunea', 'Pig', 1, password=valid_password[:-3])
...
def test_yos_must_be_between_1_and_7():
    with pytest.raises(TypeError):
        User('Gunea', 'Pig', 0, password=valid_password)
    with pytest.raises(TypeError):
        User('Gunea', 'Pig', 8, password=valid_password)
